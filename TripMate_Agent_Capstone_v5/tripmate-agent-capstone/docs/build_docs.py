...
def build_prd():
    ...
    table(doc, ["Attribute", "Requirement"], [
        ...
        ["Providers", "mock (default), OpenAI, Ollama, Groq via LLM_PROVIDER"],
    ], widths=[1.6, 5.1])
...
def build_test_plan():
    ...
    table(doc, ["In scope", "Out of scope"], [
        ...
        ["Mock LLM (default) — deterministic; OpenAI/Ollama/Groq providers by configuration", "Model quality benchmarking across vendors"],
    ...
    bullets(doc, [
        ...
        ("LLM", "mock-llm-1.0 (offline, deterministic; consent-gate defect is seeded per session so flakiness is reproducible). Optional: LLM_PROVIDER=openai|ollama|groq."),
