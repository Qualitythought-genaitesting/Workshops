...
def parse_dates(text: str) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str]]:
    ...
    if re.search(r"\btomm?orr?ow\b", low) or "kal" in low.split() or re.search(r"\bt[o0]mrw\b", low):
...
# Built from the same CITY_CODES table (which now includes the extra cities added
# in app/data.py) so "<city> to/-> <city>" routes work for every known city, not
# just the original 7 — a strict superset of the old hardcoded alternation, so
# every route that used to match still matches the same way.
_CITY_ALT = "|".join(sorted((re.escape(k) for k in CITY_CODES.keys()), key=len, reverse=True))
_ROUTE_RX = re.compile(r"\b(" + _CITY_ALT + r")\s*(?:→|->|to|se|-|–)\s*(" + _CITY_ALT + r")\b", re.I)
...
def extract(text: str) -> Constraints:
    ...
    # cities / route
    m = _ROUTE_RX.search(low)
    if m:
        c.origin, c.dest = CITY_CODES[m.group(1)], CITY_CODES[m.group(2)]
    else:
