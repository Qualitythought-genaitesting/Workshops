...
from .multi_agent import coordinator as multi_agent_coordinator
...
class ChatIn(BaseModel):
    ...
    message: str
    provider: Optional[str] = Field(None, description="Override the LLM provider for this message: mock | openai | groq | ollama. Omit to use the server default.")
...
def multi_ui():
    return FileResponse(os.path.join(STATIC, "multi.html"))
...
def chat(body: ChatIn):
    ...
    try:
        return agent.chat(body.session_id, body.user_id, body.message[:8000], provider=body.provider)
...
def api_providers():
    """Which LLM providers are configured right now — powers the chat UI's provider picker."""
    return provider_status()


@app.post("/chat/multi")
def chat_multi(body: ChatIn):
    """Multi-agent mode: a coordinator dispatches specialist agents (flight, hotel,
    weather, day-planner, budget) concurrently instead of running one ReAct loop.
    Same request/response shape as /chat, plus an `agents` field. Additive — the
    classroom single-agent build at /chat is untouched by this endpoint."""
    if not body.message.strip():
        raise HTTPException(400, "message is required")
    if body.user_id not in store.users:
        raise HTTPException(404, f"unknown user {body.user_id}")
    return multi_agent_coordinator.chat(body.session_id, body.user_id, body.message[:8000], provider=body.provider)
