...
├── app/
│   ├── erp_agent.py            ← Main agent (Mock + real LLM support)
│   └── erp_agent_mock.py       ← Pure deterministic mock (legacy)
└── docs/
    ├── ERP_AI_Agent_Testing_Presentation.pptx
    ├── ERP_AI_Agent_Requirements.docx
    └── ERP_AI_Agent_Test_Suite.xlsx   ← Complete production test suite (Blue + Red + E2E + Traceability + Bugs)
```
